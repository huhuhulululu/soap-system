...
- `is99203Ins('HF')` must be `true`, `is99203Ins('WC')` must be `false`
  ...
  <verify>Run `npx tsc --noEmit` from project root — zero errors. Manually trace defaultCptStr output against BatchView.vue INS_CPT values to confirm exact match.</verify>
