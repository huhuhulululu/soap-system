...
must_haves:
  truths:
    ...
    - "Frontend ICD catalog gains entries from shared module superset (65 entries vs previous 42/54)"
...
Purpose: Eliminate three copies of ICD data (shared 65, writer-constants 54, BatchView 42) and two copies of CPT logic, establishing src/shared/ as the single source of truth for both frontend and backend.
...
- `is99203Ins('HF')` must be `true`, `is99203Ins('WC')` must be `false`
  ...
  <verify>Run `npx tsc --noEmit` from project root — zero errors. Verify ICD entry count: `node -e "const m=require('./src/shared/icd-catalog');console.log(m.getICDCatalog().length)"` must print `65`. Manually trace defaultCptStr output against BatchView.vue INS_CPT values to confirm exact match.</verify>
...
<success_criteria>
- ICD data has ONE source of truth: src/shared/icd-catalog.ts (65 entries)
