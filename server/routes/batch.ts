...
export function createBatchRouter(): Router {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!isValidExcelBuffer(req.file.buffer)) {
          res
            .status(400)
            .json({ success: false, error: "Invalid file format" });
          return;
        }
        ...
        const mode: BatchMode =
          rawMode === "soap-only"
            ...
            : rawMode === "continue"
              ? "continue"
              : "full";
        const realisticPatch =
          req.body?.realisticPatch === true ||
          req.body?.realisticPatch === "true";
        const disableChronicCaps =
          req.body?.disableChronicCaps === true ||
          req.body?.disableChronicCaps === "true";
        ...
        // 5. Generate via worker thread (supports per-patient modes)
        const result = await generateBatchAsync(
          batchData,
          realisticPatch,
          disableChronicCaps,
        );
  ...
  router.post("/json", async (req: Request, res: Response) => {
    try {
      const {
        rows,
        mode: rawMode,
        realisticPatch: rawPatch,
        disableChronicCaps: rawChronicCaps,
      } = req.body ?? {};
      ...
      const disableChronicCaps =
        rawChronicCaps === true || rawChronicCaps === "true";
      const { patients, summary } = buildPatientsFromRows(rows, mode);
      ...
      // Always generate via worker thread (supports per-patient modes)
      const result = await generateBatchAsync(
        batchData,
        realisticPatch,
        disableChronicCaps,
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (isNaN(patientIdx) || isNaN(visitIdx)) {
          res
            .status(400)
            .json({ success: false, error: "Invalid patient or visit index" });
          return;
        }
        ...
        const realisticPatchFlag =
          req.body?.realisticPatch === true ||
          req.body?.realisticPatch === "true";
        const disableChronicCapsFlag =
          req.body?.disableChronicCaps === true ||
          req.body?.disableChronicCaps === "true";
        const regenerated = regenerateVisit(
          patient,
          visit,
          seed,
          realisticPatchFlag,
          disableChronicCapsFlag,
        );
  ...
  router.post("/:batchId/generate", async (req: Request, res: Response) => {
    try {
      ...
      const realisticPatchGen =
        req.body?.realisticPatch === true ||
        req.body?.realisticPatch === "true";
      const disableChronicCapsGen =
        req.body?.disableChronicCaps === true ||
        req.body?.disableChronicCaps === "true";
      const result = await generateBatchAsync(
        batch,
        realisticPatchGen,
        disableChronicCapsGen,
      );
