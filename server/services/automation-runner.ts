...
export async function saveCookies(storageState: unknown): Promise<void> {
  if (!existsSync(DATA_DIR)) {
    await fs.mkdir(DATA_DIR, { recursive: true });
  }
  const normalized = normalizeCookies(storageState);
  const encrypted = encrypt(JSON.stringify(normalized));
  await fs.writeFile(cookiesPath(), encrypted);
}

export async function loadCookies(): Promise<unknown> {
  const data = await fs.readFile(cookiesPath());
...
export async function decryptCookiesToTempFile(): Promise<string> {
  const plain = JSON.stringify(normalizeCookies(await loadCookies()), null, 2);
  await fs.writeFile(TEMP_COOKIES, plain, { mode: 0o600 });
  return TEMP_COOKIES;
}

export async function cleanupTempCookies(): Promise<void> {
  try {
...
export async function getCookiesInfo(): Promise<{
  exists: boolean;
...
export async function startAutomation(
  ...
  if (isRunning()) {
    throw new Error(
      `Automation already running for batch ${currentJob!.batchId}`,
    );
  }

  if (!(await hasCookies())) {
    throw new Error(
      "MDLand cookies not uploaded. Please upload storage state first.",
    );
  }
  ...
  const args = [
    ...
    apiBase,
    "--state",
    await decryptCookiesToTempFile(),
  ];
