...
import { TEMPLATE_MUSCLES } from "../shared/template-options";
...
export function weightPainTypes(
  ...
  return options.map((option) => {
    ...
    if (pattern) {
      ...
      // 寒证 -> 冷痛
      if (
        (context.localPattern.includes("Cold") ||
          context.localPattern.includes("Yang Deficiency")) &&
        option === "Freezing"
      ) {
        weight += 30;
        reasons.push("寒证常见冷痛");
...
export function weightMuscles(
  ...
  // Derive preferred muscles from canonical TEMPLATE_MUSCLES (tightness list)
  const templateEntry =
    TEMPLATE_MUSCLES[context.bodyPart as keyof typeof TEMPLATE_MUSCLES];
  const preferredMuscles: readonly string[] = templateEntry
    ? templateEntry.tightness
    : [];
  ...
  return options.map((option) => {
    ...
    if (
      preferredMuscles.some((m) =>
        option.toLowerCase().includes(m.toLowerCase()),
      )
    ) {
      weight += 40;
...
export function weightAdl(
  ...
  return options.map((option) => {
    ...
    // 匹配身体部位相关活动
    if (
      preferredAdl.some((adl) =>
        option.toLowerCase().includes(adl.toLowerCase()),
      )
    ) {
      weight += 30;
    ...
    // 严重程度越高，越偏好功能受限大的活动
    if (
      context.severityLevel === "severe" ||
      context.severityLevel === "moderate to severe"
    ) {
      if (
        option.toLowerCase().includes("stair") ||
        option.toLowerCase().includes("rising") ||
        option.toLowerCase().includes("bending")
      ) {
        weight += 10;
...
export function calculateWeights(
  ...
  if (
    fieldPath.includes("treatmentPrinciples") ||
    fieldPath.includes("focusOn")
  ) {
  ...
  if (
    fieldPath.includes("electrical") ||
    (options.includes("with") && options.includes("without"))
  ) {
