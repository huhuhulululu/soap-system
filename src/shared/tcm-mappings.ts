...
export function isTonguePatternConsistent(
  ...
  const entry = Object.entries(TCM_PATTERNS).find(([key]) =>
    lp.includes(key.toLowerCase()),
  );
...
export function getPatternTonguePulse(
  ...
  if (!pattern) {
    ...
    const entry = Object.entries(TCM_PATTERNS).find(
      ([key]) =>
        key.toLowerCase().includes(patternName.toLowerCase()) ||
...
export function getAllPatternTonguePulse(): Record<
  string,
...
export interface PainTypeCheckResult {
  consistent: true; // always true — soft validation
  warning?: string; // present when types don't match expected
  expected: string[]; // expected pain types for the pattern
}

/** Check pain types against pattern — always accepts, warns on mismatch */
export function isPainTypeConsistentWithPattern(
  localPattern: string,
  painTypes: string[],
): PainTypeCheckResult {
  const entry = Object.entries(PAIN_TYPE_BY_PATTERN).find(([key]) =>
    localPattern.includes(key),
  );
  if (!entry) return { consistent: true, expected: [] };

  const expected = entry[1];
  const hasOverlap = painTypes.some((p) =>
  ...
  return {
    ...
    ...(hasOverlap
      ? {}
      : {
          warning: `Patient-reported pain types [${painTypes.join(", ")}] differ from expected [${expected.join(", ")}] for pattern "${entry[0]}"`,
        }),
  };
}
