...
function checkSingleVisit(
  ...
  if (
    visit.severityLevel !== expectedSev &&
    !(
  ...
  if (ctx.hasPacemaker && visit.electricalStimulation) {
    errors.push({
      ...
      expected: "no electrical stimulation",
      actual: "with electrical stimulation",
    });
  }
  ...
  if (
    pain >= 7 &&
    !tightLower.includes("moderate") &&
    !tightLower.includes("severe")
  ) {
    errors.push({
      ruleId: "X1",
      severity: "CRITICAL",
      ...
      expected: "moderate or severe",
      actual: visit.tightnessGrading,
    });
  }
  if (pain <= 4 && (tightLower.includes("severe") || actualTender > 2)) {
    errors.push({
      ...
      expected: "mild tightness, +1~+2 tenderness",
      actual: `${visit.tightnessGrading}/+${actualTender}`,
    });
  }
  ...
  if (visit.isIE && ctx.localPattern) {
    if (
      !isTonguePatternConsistent(
        ctx.localPattern,
        visit.tonguePulse.tongue,
        visit.tonguePulse.pulse,
      )
    ) {
      errors.push({
        ruleId: "X3",
        ...
        visitIndex: idx,
        message: "舌脉与证型不一致",
        expected: `match ${ctx.localPattern}`,
        actual: `${visit.tonguePulse.tongue} / ${visit.tonguePulse.pulse}`,
      });
    }
  }
  ...
  const isImprovement = visit.symptomChange
    .toLowerCase()
  ...
  for (let i = 1; i < visits.length; i++) {
    ...
    if (cur.painScaleCurrent > prev.painScaleCurrent) {
      errors.push({
        ...
        visitIndex: idx,
        message: "疼痛分值回升",
        expected: `<= ${prev.painScaleCurrent}`,
        actual: String(cur.painScaleCurrent),
      });
    }
    ...
    if (curTightRank > prevTightRank) {
      errors.push({
        ...
        expected: `<= ${prev.tightnessGrading}`,
        actual: cur.tightnessGrading,
      });
    }
    ...
    if (curFreq > prevFreq) {
      errors.push({
        ...
        visitIndex: idx,
        message: "疼痛频率增加",
        expected: `<= ${prev.painFrequency}`,
        actual: cur.painFrequency,
      });
    }
    ...
    if (
      severityToRank(cur.severityLevel) > severityToRank(prev.severityLevel)
    ) {
      errors.push({
        ruleId: "T08",
        severity: "HIGH",
        ...
        expected: `<= ${prev.severityLevel}`,
        actual: cur.severityLevel,
      });
    }
    ...
    if (curSymRank > prevSymRank && prevSymRank > 0) {
      errors.push({
        ...
        visitIndex: idx,
        message: "伴随症状严重程度恶化",
        expected: `<= ${prev.associatedSymptom}`,
        actual: cur.associatedSymptom,
      });
    }
    ...
    if (cur.symptomChange.toLowerCase().includes("improvement")) {
      ...
      if (painWorsened || tenderWorsened || tightWorsened) {
        ...
        if (painWorsened)
          indicators.push(
            `pain: ${prev.painScaleCurrent}→${cur.painScaleCurrent}`,
          );
        if (tenderWorsened)
          indicators.push(`tenderness: +${prevTender}→+${curTender}`);
        if (tightWorsened)
          indicators.push(
            `tightness: ${prev.tightnessGrading}→${cur.tightnessGrading}`,
          );
        errors.push({
          ruleId: "T02",
          severity: "CRITICAL",
...
function scoreErrors(
  ...
  ValidationResult,
  "score" | "grade" | "criticalCount" | "highCount" | "mediumCount"
> {
...
export function visitStateToSnapshot(state: {
  visitIndex: number;
  painScaleCurrent: number;
  severityLevel: string;
  symptomChange: string;
...
export function contextToSnapshot(ctx: {
  painCurrent: number;
  localPattern: string;
  systemicPattern: string;
  painTypes: string[];
