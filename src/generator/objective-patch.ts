...
function bumpStrength(strength: string, progress: number): string {
  const step = progress > 0.8 ? 2 : progress > 0.5 ? 1 : 0;
...
function patchedComputeRom(
  ...
  if (overrideStrengthGrade) {
    ...
    const STRENGTH_SENS: Record<ROMDifficulty, number> = {
      HARD: 1.3,
      MEDIUM: 1.0,
      EASY: 0.7,
    };
    const jitter = (((index * 3 + sideOffset) % 5) - 2) * 0.3;
    const strengthPain = clamp(
      adjustedPain * STRENGTH_SENS[rom.difficulty] + jitter,
      1,
      10,
    );

    strength = strengthFromPain(strengthPain);
    strength = applyConditionDowngrade(
      strength,
      baselineCondition,
      chronicityLevel,
    );
  ...
  const romPain = clamp(
    adjustedPain * getPainSensitivity(rom.difficulty),
    1,
    10,
  );
 ...
 */
function deriveBaselineCondition(
...
function getShoulderRomOptions(
  movement: string,
): ReadonlyArray<{ degrees: number; label: string }> | null {
  if (movement === "Abduction") return SHOULDER_ABDUCTION_OPTIONS;
  if (movement === "Horizontal Adduction")
    return SHOULDER_HORIZONTAL_ADDUCTION_OPTIONS;
  if (movement === "Flexion") return SHOULDER_FLEXION_OPTIONS;
  if (movement === "Extension") return SHOULDER_EXTENSION_OPTIONS;
  if (movement === "External Rotation")
    return SHOULDER_EXTERNAL_ROTATION_OPTIONS;
  if (movement === "Internal Rotation")
    return SHOULDER_INTERNAL_ROTATION_OPTIONS;
  return null;
}
...
export function buildPatchContext(
  ...
  visitState?: {
    ...
    strengthGrade?: string;
    soaChain?: {
      objective?: { romTrend?: string };
      subjective?: { adlChange?: string };
    };
  },
): PatchContext {
  ...
  return {
    ...
    adlImproved:
      context.noteType === "TX" &&
      visitState?.soaChain?.subjective?.adlChange === "improved",
...
function regenerateGenericRomLine(
  ...
  const { strength, romValue, limitation } = patchedComputeRom(
    ...
    romAdj,
    pctx.progress,
    pctx.baselineCondition,
    pctx.chronicityLevel,
    pctx.adlImproved,
    pctx.strengthGrade,
  );
 ...
 * 生成完整的 ROM 区块文本（补丁版）
 */
function generatePatchedRomBlock(
  pctx: PatchContext,
  visitState?: {
    progress?: number;
    soaChain?: { objective?: { romTrend?: string } };
  },
): string {
  ...
  const painLevel: number =
    pctx.isTX && pctx.progress !== undefined
  ...
  if (bp === "KNEE" && pctx.laterality === "bilateral") {
    ...
    for (let i = 0; i < romData.length; i++) {
      block +=
        regenerateKneeRomLine(romData[i], i, 0, painLevel, 0, pctx) + "\n";
    }
    block += "\n";
  } else if (bp === "SHOULDER") {
...
export function patchSOAPText(
  ...
  // 重新组装 Objective
  let result =
    fullText.slice(0, objContentStart) + objectiveText + fullText.slice(objEnd);
...
export function computePatchedGoals(
  ...
  if (condition === "poor") {
    ...
    goals.strength = {
      st: capStr(goals.strength.st),
      lt: capStr(goals.strength.lt),
    };
    goals.rom = {
      ...goals.rom,
      lt: `${Math.max(40, parseRomPct(goals.rom.lt) - 5)}%`,
