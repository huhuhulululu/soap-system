...
describe('维度3: progress系数', () => {
  ...
  it('下限 0.70', () => {
    const mult = inferProgressMultiplier(['Stroke', 'Parkinson', 'Diabetes', 'Kidney Disease', 'Anemia'], 80)
    expect(mult).toBeGreaterThanOrEqual(0.70)
  })
})
...
describe('维度4: 初始值修正', () => {
  it('Herniated Disk + LBP → severity+1, ROM+0.10', () => {
    ...
    expect(adj.romDeficitBump).toBeCloseTo(0.10)
  })
...
describe('维度5: 特殊约束', () => {
  ...
  it('Joint Replacement → cautionElectricalStim', () => {
    expect(inferConstraints(['Joint Replacement']).cautionElectricalStim).toBe(true)
  })
...
describe('引擎集成', () => {
  ...
  it('Parkinson 患者 spasm 基线更高', () => {
    ...
    const extractSpasm = (g: string) => { const m = g.match(/\(([+]?\d)\)/); return m ? parseInt(m[1].replace('+', '')) : 0 }
    expect(extractSpasm(r2.states[0].spasmGrading)).toBeGreaterThanOrEqual(extractSpasm(r1.states[0].spasmGrading))
  })

  it('inferMedicalProfile 综合输出完整', () => {
    const profile = inferMedicalProfile(['Diabetes', 'Stroke', 'Pacemaker'], 70, 'LBP', 'Qi & Blood Deficiency')
