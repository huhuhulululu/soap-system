...
import {
  isAdlConsistentWithBodyPart,
  ADL_ACTIVITIES,
} from "../../../../src/shared/adl-mappings";
...
describe("tcm-mappings.ts", () => {
  describe("isTonguePatternConsistent", () => {
    ...
    it("Qi Stagnation + red yellow coat + rapid → inconsistent", () => {
      expect(
        isTonguePatternConsistent("Qi Stagnation", "red, yellow coat", "rapid"),
      ).toBe(false);
    });
  });
  ...
  describe("isPainTypeConsistentWithPattern", () => {
    ...
    it("Blood Stasis + Dull only → soft warning (always consistent)", () => {
    ...
    it("Qi Stagnation + Dull/Aching → consistent", () => {
      const result = isPainTypeConsistentWithPattern("Qi Stagnation", [
        "Dull",
        "Aching",
      ]);
      expect(result.consistent).toBe(true);
...
describe("adl-mappings.ts", () => {
  ...
  describe("isAdlConsistentWithBodyPart", () => {
    it("LBP + Standing for long periods → consistent", () => {
      const result = isAdlConsistentWithBodyPart(
        "LBP",
        "difficulty with Standing for long periods of time",
      );
      expect(result.consistent).toBe(true);
    });
    ...
    it("SHOULDER + holding the pot → consistent", () => {
      const result = isAdlConsistentWithBodyPart(
        "SHOULDER",
        "difficulty with holding the pot for cooking",
      );
      expect(result.consistent).toBe(true);
    });
    it("NECK + Tilting head → consistent", () => {
      const result = isAdlConsistentWithBodyPart(
        "NECK",
        "difficulty with Tilting head to talking the phone",
      );
      expect(result.consistent).toBe(true);
    });
