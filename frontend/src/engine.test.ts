...
import { calculateDynamicGoals, parsePainFromSeverity } from '../../src/generator/goals-calculator'
import { setWhitelist } from '../../src/parser/template-rule-whitelist.browser'
import { filterADLByDemographics, ADL_MUSCLE_MAP, MUSCLE_SEVERITY_ORDER } from '../../src/generator/soap-generator'
...
describe('Phase 1: 数据贯通', () => {
  ...
  describe('1.1 Severity 从 Pain 推导', () => {
    it('Pain 9-10 → severe', () => {
      // severityFromPain 在引擎内部, 通过 goals calculator 间接验证
      const goals = calculateDynamicGoals('severe', 'LBP', 'soreness', 9)
      expect(goals.pain.st).toBeDefined()
    })

    it('Goals 使用实际 pain 而非 severity 反推', () => {
      // pain=6 但 severity=moderate to severe (不匹配) 
      // Goals 应该基于 pain=6 计算, 而非 parsePainFromSeverity('moderate to severe')=8
      const goalsWithOverride = calculateDynamicGoals('moderate to severe', 'LBP', 'soreness', 6)
      const goalsWithoutOverride = calculateDynamicGoals('moderate to severe', 'LBP', 'soreness')
      // pain=6 的 ST goal 应该比 pain=8 的小
      expect(goalsWithOverride.pain.st).not.toBe(goalsWithoutOverride.pain.st)
    })

    it('parsePainFromSeverity 仍可正常工作', () => {
  ...
  describe('1.3 TX initialState 继承用户输入', () => {
    it('Pain 起始值继承 initialState', () => {
      ...
      const { states } = generateTXSequenceStates(ctx, {
        txCount: 3, startVisitIndex: 1, seed: SEED,
    ...
    it('SymptomScale 起始值继承', () => {
      ...
      const { states } = generateTXSequenceStates(ctx, {
        txCount: 3, startVisitIndex: 1, seed: SEED,
        initialState: { pain: 8, symptomScale: '50%', associatedSymptom: 'soreness' }
      })
      // symptomScale 从 50% 开始递减, TX1 应 ≤ 50%
      const pct = parseInt(states[0].symptomScale?.replace('%', '') || '50', 10)
    ...
    it('PainTypes 继承用户输入', () => {
      ...
      const { states } = generateTXSequenceStates(ctx, {
        txCount: 3, startVisitIndex: 1, seed: SEED,
        initialState: { pain: 8, painTypes: ['Burning', 'Stabbing'], associatedSymptom: 'soreness' }
      })
...
describe('Phase 2: 引擎逻辑优化', () => {
  ...
  describe('2.1 TX1 使用统一康复曲线', () => {
    it('TX1 不强制特定降幅', () => {
      ...
      for (let s = 1; s <= 10; s++) {
        const { states } = generateTXSequenceStates(ctx, {
          txCount: 3, startVisitIndex: 1, seed: s,
  ...
  describe('2.2 Spasm 有随机因子', () => {
    it('不同 seed 的 spasm 变化时间点不同', () => {
      ...
      for (let s = 1; s <= 5; s++) {
        const { states } = generateTXSequenceStates(ctx, {
          txCount: 11, startVisitIndex: 1, seed: s,
          initialState: { pain: 8, associatedSymptom: 'soreness' }
        })
        spasmSequences.push(states.map(st => st.spasmGrading))
      }
      // 不同 seed 至少有一对 spasm 序列不同
      const allSame = spasmSequences.every(seq =>
        JSON.stringify(seq) === JSON.stringify(spasmSequences[0])
      )
  ...
  describe('2.3 Tightness/Tenderness 初始值从 pain 推导', () => {
    ...
    it('Pain 4 → 初始 tightness 约 2 (mild to moderate)', () => {
      const ctx = makeContext({ painCurrent: 4, severityLevel: 'mild to moderate' })
      const { states } = generateTXSequenceStates(ctx, {
        txCount: 3, startVisitIndex: 1, seed: SEED,
  ...
  describe('2.4 ROM/Strength 独立进度', () => {
    ...
    it('Strength 滞后于 ROM', () => {
      ...
      const romFirstImprove = states.findIndex(s => s.soaChain.objective.romTrend !== 'stable')
      const stFirstImprove = states.findIndex(s => s.soaChain.objective.strengthTrend !== 'stable')
...
describe('Phase 3: ADL/Muscle 模型', () => {
  ...
  describe('3.4 ADL 年龄+性别过滤', () => {
    it('高龄男性排除 "holding the pot for cooking"', () => {
      const shoulderADL = [
        'holding the pot for cooking', 'performing household chores',
        'working long time in front of computer', 'put on/take off the clothes'
      ]
  ...
  describe('3.5 ADL-Muscle 关联映射', () => {
    ...
    it('SHOULDER "reach top of cabinet" 映射到 supraspinatus', () => {
      const muscles = ADL_MUSCLE_MAP['SHOULDER']['reach top of cabinet to get object(s)']
...
describe('各部位 Objective', () => {
  const bodyParts = ['LBP', 'NECK', 'SHOULDER', 'KNEE', 'ELBOW', 'HIP'] as const

  for (const bp of bodyParts) {
    it(`${bp} IE 生成不报错`, () => {
      const ctx = makeContext({ noteType: 'IE', primaryBodyPart: bp, painCurrent: 7 })
  ...
  it('SHOULDER bilateral 输出左右 ROM', () => {
    const ctx = makeContext({ noteType: 'TX', primaryBodyPart: 'SHOULDER', laterality: 'bilateral' })
    const { states } = generateTXSequenceStates(ctx, {
      txCount: 1, startVisitIndex: 1, seed: SEED,
      initialState: { pain: 8, associatedSymptom: 'soreness' }
    })
  ...
  it('SHOULDER left 只输出左侧 ROM', () => {
    const ctx = makeContext({ noteType: 'TX', primaryBodyPart: 'SHOULDER', laterality: 'left' })
    const { states } = generateTXSequenceStates(ctx, {
      txCount: 1, startVisitIndex: 1, seed: SEED,
      initialState: { pain: 8, associatedSymptom: 'soreness' }
    })
