...
import {
  isTonguePatternConsistent,
  isPainTypeConsistentWithPattern,
} from "../../../src/shared/tcm-mappings";
...
function isTonePatternConsistent(
...
function checkTX(
  ...
  const withinTwoLevelsDown =
    expectedIdx >= 0 &&
    actualIdx >= 0 &&
    expectedIdx - actualIdx <= 2 &&
    expectedIdx - actualIdx >= 0;
  ...
  if (prevVisit && !prevIsIE) {
    ...
    if (saysImprove) {
      ...
      const tightnessWorsened =
        compareSeverity(curTightness, prevTightness) > 0;
      ...
      if (painWorsened || tendernessWorsened || tightnessWorsened) {
        ...
        if (tendernessWorsened)
          worsenedIndicators.push(
            `tenderness: +${prevTenderness}→+${curTenderness}`,
          );
        if (tightnessWorsened)
          worsenedIndicators.push(
            `tightness: ${prevTightness}→${curTightness}`,
          );
  ...
  const progressReasons = extractProgressReasons(
    visit.subjective.chiefComplaint,
  );
...
function checkSequence(visits: VisitRecord[]): CheckError[] {
  ...
  for (let i = 1; i < visits.length; i++) {
    ...
    if (adlSeverityDelta > 1) {
      errors.push(
        err({
          ...
          expected: `<= 1 level above ${prevAdlSeverity}`,
          actual: curAdlSeverity,
        }),
      );
    }
    ...
    const prevSymptom = extractAssociatedSymptom(
      prev.subjective.chiefComplaint,
    );
      ...
      if (curRank > prevRank + 1) {
        errors.push(
          err({
            ...
            expected: `<= ${prevSymptom}(${prevRank})+1`,
            actual: `${curSymptom}(${curRank})`,
          }),
        );
      }
    }
...
function penalty(
  sev: RuleSeverity,
  table: Record<RuleSeverity, number>,
): number {
  return table[sev];
}
        ...
        if (/\d/.test(lastChar) && !expectedSuffixes.includes(lastChar)) {
          errors.push(
            err({
              ...
              expected: `suffix ${expectedSuffixes.join("/")}`,
              actual: lastChar,
            }),
          );
        }
...
function checkTextConsistency(visits: VisitRecord[]): CheckError[] {
  ...
  for (let i = 0; i < visits.length; i++) {
    ...
    for (const noun of NEGATIVE_NOUNS) {
      const increasedPattern = new RegExp(
        `increased\\s+(?:\\w+\\s+)*${noun}`,
        "i",
      );
    ...
    for (const noun of POSITIVE_NOUNS) {
      if (noun === "rom") {
        ...
        const reducedRomLimitationPattern =
          /reduced\s+(?:\w+\s+)*rom\s+limitation/i;
        const reducedRomPattern =
          /reduced\s+(?:\w+\s+)*(?:joint\s+)?rom(?!\s+limitation)/i;
        if (
          reducedRomPattern.test(fullText) &&
          !reducedRomLimitationPattern.test(fullText)
        ) {
          errors.push(
            err({
              ruleId: "T01",
        ...
        const reducedPattern = new RegExp(
          `reduced\\s+(?:\\w+\\s+)*${noun}`,
          "i",
        );
    ...
    if (i > 0) {
      ...
      const reducedRomLimitationPattern =
        /reduced\s+(?:\w+\s+)*(?:rom\s+)?limitation/i;
      const increasedRomPattern =
        /increased\s+(?:\w+\s+)*(?:joint\s+)?rom(?!\s+limitation)/i;
      ...
      if (increasedStrengthPattern.test(fullText) && curStr < prevStr) {
        errors.push(
          err({
            ...
            ruleName: "肌力描述矛盾",
            message:
              '文本描述 "increased muscles strength" 但 strength 实际下降',
...
function checkGeneratorRules(
  ...
  for (let i = 0; i < visits.length; i++) {
    ...
    // S2: painTypes vs localPattern — soft warning (patient-reported pain always accepted)
    if (v.subjective.painTypes.length > 0 && v.assessment.localPattern) {
      const { warning, expected } = isPainTypeConsistentWithPattern(
        v.assessment.localPattern,
        v.subjective.painTypes,
      );
      if (warning) {
        errors.push(
          err({
            ...
            actual: v.subjective.painTypes.join(","),
          }),
        );
      }
    }
    ...
    if (v.subjective.adlImpairment && v.subjective.bodyPartNormalized) {
      const { consistent, keywords } = isAdlConsistentWithBodyPart(
        v.subjective.bodyPartNormalized,
        v.subjective.adlImpairment,
      );
    ...
    if (v.subjective.bodyPartNormalized) {
      ...
      const keywords =
        muscleKeywords[v.subjective.bodyPartNormalized.toUpperCase()] || [];
      if (keywords.length > 0) {
        const allMuscles = [
          ...v.objective.tightnessMuscles.muscles,
          ...v.objective.tendernessMuscles.muscles,
          ...v.objective.spasmMuscles.muscles,
        ];
    ...
    if (v.subjective.bodyPartNormalized) {
      ...
      const valid =
        validMovements[v.subjective.bodyPartNormalized.toUpperCase()] || [];
      if (valid.length > 0) {
        for (const rom of v.objective.rom.items) {
    ...
    if (!isIE && i > 0) {
      ...
      if (ieVisit?.assessment.localPattern && v.assessment.localPattern) {
        ...
        if (
          !curPattern.includes(iePattern) &&
          !iePattern.includes(curPattern)
        ) {
          errors.push(
            err({
              ruleId: "A5",
    ...
    if (v.subjective.bodyPartNormalized) {
      ...
      const valid =
        validGauges[v.subjective.bodyPartNormalized.toUpperCase()] || [];
      if (valid.length > 0) {
    ...
    if (
      !v.plan.acupoints ||
      v.plan.acupoints.length === 0 ||
      v.plan.acupoints.length > 20
    ) {
      errors.push(
        err({
          ruleId: "P2",
          ...
          expected: "2-20",
          actual: String(v.plan.acupoints?.length || 0),
        }),
      );
    }
    ...
    if (!isIE) {
      ...
      const actualTightness =
        v.objective.tightnessMuscles.gradingScale.toLowerCase();
    ...
    if (isIE && (v.assessment.localPattern || v.assessment.systemicPattern)) {
      ...
      const patterns = [
        v.assessment.localPattern,
        v.assessment.systemicPattern,
      ].filter(Boolean);
      const tongue = v.objective.tonguePulse.tongue.toLowerCase();
      let anyPatternMatched = false;
      for (const pattern of patterns) {
        const expectedTongues = Object.entries(patternTongue).find(([key]) =>
          pattern!.includes(key),
        )?.[1];
        if (
          expectedTongues &&
          expectedTongues.some((t) => tongue.includes(t))
        ) {
      ...
      if (!anyPatternMatched && patterns.length > 0) {
        const allExpected = patterns.flatMap((p) => {
          const e = Object.entries(patternTongue).find(([key]) =>
            p!.includes(key),
          )?.[1];
          return e || [];
        });
        if (allExpected.length > 0) {
          errors.push(
            err({
              ...
              actual: v.objective.tonguePulse.tongue,
            }),
          );
        }
      }
    ...
    const hasPacemaker =
      v.subjective.medicalHistory?.some((h) =>
        h.toLowerCase().includes("pacemaker"),
...
export function checkDocument(input: CheckInput): CheckOutput {
  ...
  visits.forEach((visit, idx) => {
    if (visit.subjective.visitType === "INITIAL EVALUATION") {
      ...
      errors.push(
        ...checkTX(visit, ieVisit, idx > 0 ? visits[idx - 1] : undefined, idx),
      );
    }
  });
  ...
  errors.push(
    ...checkGeneratorRules(
      visits,
      input.insuranceType || "OPTUM",
      input.treatmentTime || 15,
    ),
  );
